import { createWalletConnection, verifyToken, isVerifiedWallet } from "../../db/sql/verificationQueries"
...
export const verifyWallet = async (nftCollections: NftWalletCollection[], verifiedCollections: VerificationCollection[], walletAddress: string, connection: Database): Promise<WalletVerification> => {
    try {
        const walletVerified = await isVerifiedWallet(walletAddress, connection)
        let verified = undefined
        if(!walletVerified) {
            verified = verifiedCollections.find(verifiedCollection => {
